            ...
            homeContent: function (filter) {
                ...
                if (tid === "genre?") {
                    ...
                    if ($.isNumeric(key)) {
                        ...
                        $(".video_grid_container .grid_0_cell").each(function () {
                            let match = $(this).find(".grid_0_title").text().match(/^\w+-\w+/g);
                            if (!match) {
                                match = $(this).find(".grid_0_title").text().match(/\w+-\w+$/g);
                            }
                            let picId = "";
                            if (match) {
                                picId = match.at(0).toLowerCase().replace("-", "00");
                            }
                            const params = new URLSearchParams($(this).find("a").eq(0).attr("href").split("?").at(1));
                            result.list.push({
                                vod_id: params.get("id"),
                                vod_name: $(this).find(".grid_0_title").text(),
                                vod_pic: `https://pics.dmm.co.jp/digital/video/${picId}/${picId}pl.jpg`,
                                vod_remarks: $(this).find(".grid_0_date").text(),
                            });
                        });
                    } else {
